...
public class DoctorPendingApprovalView {
    ...
    // =========================================================================
    // LEFT MARKETING PANEL (Consistent Across Auth Flow)
    // =========================================================================
    ...
    // =========================================================================
    // RIGHT PANEL - DOCTOR PENDING APPROVAL CARD
    // =========================================================================
    private ScrollPane createRightFormPanel() {
        ...
        // 2. Illustration (Shield + Clock + Document)
        Node illustrationNode = createApprovalIllustration();

        // 3. Heading
        Text headingText = new Text("Application Submitted");
        headingText.getStyleClass().add("heading-text");

        // 4. Subtitle
        Text subtitleText = new Text("Thank you for registering. Our verification team will review your submitted credentials.");
        subtitleText.getStyleClass().add("subtitle-text");
        subtitleText.setStyle("-fx-text-alignment: center;");

        // 5. Verification Time Badge
        HBox timeBadge = createTimeBadge();
        ...
        // 10. Support Link
        Hyperlink supportLink = new Hyperlink("Need help? Contact Support");
        supportLink.setStyle("-fx-text-fill: #6B7280; -fx-font-size: 13px; -fx-underline: false;");
    ...
    // =========================================================================
    // CUSTOM ILLUSTRATION (160x160)
    // =========================================================================
    private Node createApprovalIllustration() {
        ImageView illustrationImg = createSafeImageView("/images/pending-approval.png", 160, 160);
        ...
        Circle outerCircle = new Circle(70, Color.web("#EFF6FF"));
        Circle innerCircle = new Circle(52, Color.web("#DBEAFE"));

        Text shieldIcon = new Text("🛡️");
        shieldIcon.setStyle("-fx-font-size: 48px;");

        Text clockBadge = new Text("⏳");
        clockBadge.setStyle("-fx-font-size: 22px;");

        StackPane.setAlignment(clockBadge, Pos.BOTTOM_RIGHT);
        StackPane.setMargin(clockBadge, new Insets(0, 24, 24, 0));

        illustrationPane.getChildren().addAll(outerCircle, innerCircle, shieldIcon, clockBadge);
    ...
    private HBox createTimeBadge() {
        ...
        badge.setStyle("-fx-background-color: #F3F4F6; -fx-background-radius: 20px; -fx-border-color: #E5E7EB; -fx-border-radius: 20px;");

        Label labelText = new Label("Estimated verification time:");
        labelText.setStyle("-fx-text-fill: #4B5563; -fx-font-size: 13px;");

        Label timeText = new Label("24–48 Hours");
        timeText.setStyle("-fx-text-fill: #2563EB; -fx-font-size: 13px; -fx-font-weight: bold;");

        badge.getChildren().addAll(labelText, timeText);
        return badge;
    }
    ...
    private VBox createStatusCard() {
        ...
        Text headerTitle = new Text("Verification Status");
        headerTitle.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-fill: #1F2937;");

        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);

        Label pendingBadge = new Label("Pending Review");
        pendingBadge.setStyle("-fx-background-color: #F3E8FF; -fx-text-fill: #7E22CE; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 4px 10px; -fx-background-radius: 12px;");
        ...
        HBox row2 = createChecklistRow("⏳", "#6B7280", "Professional verification pending", "#374151");
        HBox row3 = createChecklistRow("✉", "#2563EB", "Email notification after approval", "#6B7280");

        checklist.getChildren().addAll(row1, row2, row3);
        card.getChildren().addAll(header, divider, checklist);

        return card;
    }

    private HBox createChecklistRow(String icon, String iconColor, String text, String textColor) {
        ...
        Text descText = new Text(text);
        descText.setStyle("-fx-font-size: 13px; -fx-fill: " + textColor + ";");

        row.getChildren().addAll(iconText, descText);
        return row;
    }
    ...
    private HBox createWhyVerificationCard() {
        ...
        card.setAlignment(Pos.CENTER_LEFT);
        card.setStyle("-fx-background-color: #EFF6FF; -fx-border-color: #BFDBFE; -fx-border-width: 1px; -fx-border-radius: 12px; -fx-background-radius: 12px;");

        Text infoIcon = new Text("ℹ️");
        infoIcon.setStyle("-fx-font-size: 20px;");

        VBox textContainer = new VBox(4);
        Text title = new Text("Why Verification?");
        title.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-fill: #1E40AF;");

        Text body = new Text("Every Doctor and Hospital is verified to maintain patient safety.");
        body.setStyle("-fx-font-size: 13px; -fx-fill: #3B82F6;");

        textContainer.getChildren().addAll(title, body);
        card.getChildren().addAll(infoIcon, textContainer);
    ...
    private void addBtnAnimations(Button btn, Color glowColor) {
